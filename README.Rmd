...
data.frame(light.intervals = seq(0.1, 1, length.out = 10)) %>%  # Choose light levels to calculate depths for
 mutate(depths = depth(light.intervals, KdPAR = 0.03),  # Calculate depths specifying light attenuation
        community = shallow(light.intervals * 100)) -> depth.distribution

ggplot(depth.distribution) +
